...
pub fn p() -> Result<String> {
    ...
    The solution to this problem was to play multiple games and check the most popular squares
    */
    const NB_ROLLS: usize = 2e6 as usize;
    let mut game = Game::new(NB_ROLLS);
    game.play();
    let mut probability_per_squared = (0..NB_SQUARES)
        .zip(game.nb_visit_per_square)
        .collect::<Vec<(usize, usize)>>();
    probability_per_squared.sort_by_key(|x| x.1);
    let three_most_popular_squares = probability_per_squared
        .iter()
        .rev()
        .take(3)
