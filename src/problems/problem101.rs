...
fn p() -> String {
    ...
    assume a linear relationship and predict the next term to be 15 (common difference 7). Even if we were presented with the
    first three terms, by the same principle of simplicity, a quadratic relationship should be assumed.
    ...
    It should be clear that OP(k, n) will accurately generate the terms of the sequence for n ≤ k, and potentially the first
    ...
    */
    for n in 1_u128..10 {
        let _un = 1 + n.pow(2) + n.pow(4) + n.pow(6) + n.pow(8) + n.pow(10)
            - n
            - n.pow(3)
            - n.pow(5)
            - n.pow(7)
