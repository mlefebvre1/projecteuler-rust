...
mod test {
    ...
    fn test_octagonal_usize() {
        ...
        assert_eq!(
            actual,
            [
                0, 1, 8, 21, 40, 65, 96, 133, 176, 225, 280, 341, 408, 481, 560, 645, 736, 833,
                936, 1045,
            ]
        );
    }
    ...
    fn test_octagonal_biguint() {
        ...
        let actual =
            Octagonal::from_n(&BigUint::parse_bytes(b"573147844013817084101", 10).unwrap());
        let expected =
            BigUint::parse_bytes(b"985495353293060399776555409215134686766401", 10).unwrap();
        assert_eq!(actual, expected);
    }
}
